...
int linie = 6, coloana = 3;  //linie, coloana = coordonatele jucatorului in punctul X
...
int scor = 0, vieti = 10, dificultate = 1;
bool jocInceput = false;  //jocInceput = false initial. Se face true cand trec de partea cu meniuri
bool primulLoop, primulLoopDinJoc;
int randMeniu;   //rand selectat din meniul de la inceputul jocului
int stareButonPrecedenta = 0, stareButon = 0, ultimulDebounce = 0;
...
void afiseazaMeniu()
{
  /*
    ...
    selectez randul cu butonul: daca e pe randul 0 incepe jocul
  ...
  */
  ...
  if (randMeniu == 0)
  {
    ...
    lcd.print(">");
    lcd.setCursor(0, 1);
    lcd.print(" ");// "sterg" sageata de pe celalalt rand
  ...
  int stare = digitalRead(PIN_BUTON);
  // 1 cand e apasat, 0 pe liber

  if (stare != stareButonPrecedenta)
  {
  ...
  if ((millis() - ultimulDebounce) > intervalDebounce)
  {
    if (stare != stareButon)
    {
      ...
      if (stareButon == 1 && randMeniu == 0)
      {
        jocInceput = true;
      }

      if (stareButon == 1 && randMeniu == 1)
      {
        if (dificultate < 7)
          dificultate++;
        else
...
{
  //meniu dupa ce pierzi, apare pana la un input de la buton
  int intervalClear = 600, intervalDebounce = 50;
  ...
  //verific daca a fost apasat butonul; daca da, resetez valorile jocului
  int stare = digitalRead(PIN_BUTON);
  // 1 cand e apasat, 0 pe liber
...
void genereazaObstacole()
{
  ...
  */
  ...
  bool okSaPunObstacol = true; //se face false daca pe urmatoarele linii (minimLiniiLibere) este un obstacol
  ...
  for (i = 0; i < minimLiniiLibere; i++)
    ...
    {
      okSaPunObstacol = false;
      break;
    }

  if ((okSaPunObstacol == true) && ((millis() - timpUltimulObstacol) >= delayObstacole))
  {
...
void lanseazaRachete()
{
  ...
  // cu buton
  valoareButon = digitalRead(PIN_BUTON);

  if ((valoareButon == 1) && ((millis() - timpUltimaRacheta) >= intervalIntreRachete))
  {
